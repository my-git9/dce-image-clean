...
# Optimized by fuyuan.chu@daocloud.io & jie.wan@daocloud.io & xing.wu@daocloud.io
...
# Example:
#    ./gc-registry-prefix.sh -h "http://127.0.0.1" -u "admin:password"
#    ./gc-registry-prefix.sh -h "http://127.0.0.1" -u "admin:password" -p "count=5" -d
#    ./gc-registry-prefix.sh -h "http://127.0.0.1" -u "admin:password" -p "days=7"
#    ./gc-registry-prefix.sh -h "http://127.0.0.1" -u "admin:password" -n abcd
#    ./gc-registry-prefix.sh -h "http://127.0.0.1" -u "admin:password" -r "daocloud/dao-2048"
#    ./gc-registry-prefix.sh -h "http://127.0.0.1" -u "admin:password" -a "del"
...
{
    # Check Registry Host
    [[ -z "${ACTION}" ]] && ACTION="ls" || echo -n
    echo "${ACTION}" | grep -Eq 'list|ls|clean|del|delete' || help_doc
...
while getopts ':a:h:u:p:r:n:d' OPT
do
    case ${OPT} in
        a)
            ACTION="${OPTARG}"
...
# action list show image tags
function action_list() {

  [ -f ".image_tag.txt" ] && rm -rf .image_tag.txt || echo -n
  ...
  if [ "$REPO" != "" ];then
    ...
    for tag in $tags;do
      ...
      printf "%-70s %-20s %-20s\n"  "$repo" "$tag"  "${created_time}"  >> .image_tag.txt
    done
    if [ -f .image_tag.txt ];then
      printf "%-70s %-20s  %-20s\n" "Image" "Tag" "Created"
      printf "%0.s-" {1..110}
      printf "\n"
      cat .image_tag.txt | sort -k 3
      rm .image_tag.txt
    else
    ...
    for repo in $(get_repositories);do
      if [ "$PREFIX" != "" ];then
        if [[ "$repo" =~ $PREFIX.* ]];then
          printf "%-70s %-20s\n"  "$repo" "$(get_tags $repo | wc -l)" >> .image_tag.txt
        fi
      else
        printf "%-70s %-20s\n"  "$repo" "$(get_tags $repo | wc -l)" >> .image_tag.txt
      fi
    ...
    printf "\n"
    if [ -f .image_tag.txt ];then
      cat .image_tag.txt | sort -rn -k 2
      rm -rf .image_tag.txt
    else
      echo "This namespace has no images";
    fi
